...
def user_progress_summary(request):
    ...
    percent_complete = round(completed / progress.count(),2) if progress.count() > 0 else 0
    paginator_pro = Paginator(progress, 8) 

    page_number_pro = request.GET.get('progress_page')
